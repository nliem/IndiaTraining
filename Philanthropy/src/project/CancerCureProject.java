 ...
 *
 */
public class CancerCureProject extends AbstractProject implements Donatable {
   ...
   */
  public CancerCureProject(int projectId, String name, String description, double cost) {
  ...
  public void acceptDonation(int recipient, double donation, Donor donator)
    ...
    if (donation < 0) {
      throw new InvalidDonationAmountException("Invalid donation amount. Negative donations are not accepted.");
    } else if (donation > this.oustandingCosts) {
      throw new InvalidDonationAmountException("Invalid donation amount. Donation amount exceeds the outstanding "
  ...
  public void displayDonors() {
    ...
    if (donorList.isEmpty()) {
      System.out.println("There are no donors for this project.");
    }
