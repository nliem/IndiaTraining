...
/**
 * A type of project focused on Old Age Home related issues.
 * @author natalieliem
 *
 */
public class OldAgeHomeProject extends AbstractProject implements Donatable{
   ...
   * 
   * @param projectId Unique id for this project.
   * @param name Name of this project.
   * @param description Brief description of the project.
   * @param cost Total cost of the project.
   */
  public OldAgeHomeProject(int projectId, String name, String description, double cost){
  ...
  /**
   * Accepts donation and applies it the the total collected. Adds the donor to the donor list if necessary.
   */
  @Override
  public void acceptDonation(int recipient, double donation, Donor donator) throws InvalidDonationAmountException, UnknownProductException {
    ...
    if(donation < 0 ){
    	...
    	if(recipient == 0){
    		...
    		System.out.println("Donation of $" + df.format(donation) 
			+ " for project " + super.name +  " (" + super.projectId + ") accepted.");
    	}
    	else{
    		AbstractProduct p = this.products.get(recipient);
    		if(p == null) throw new UnknownProductException("Product with id " + recipient + " does not exist.");
